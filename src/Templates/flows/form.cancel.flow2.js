...
import { deleteEvent } from '../../services/scripts/calendar2.js';
import { welcomeFlow } from "../../Templates/WelcomeFlow.js";
import { deleteFromSheet2, readSheet } from "../../services/sheetsService.js";

async function findAppointmentsByPhone(userNumber) {
    const range = "AGENDA2!A2:C";
...
const formCancelFlow2 = addKeyword("cancelar cita")
    ...
    .addAnswer("Escribe el número de la cita a cancelar", { capture: true }, async (ctx, ctxFn) => {
        ...
        if (!isNaN(userInput) && userInput > 0 && userInput <= citas.length) {
            ...
            try {
                ...
                await deleteFromSheet2(eventId);
