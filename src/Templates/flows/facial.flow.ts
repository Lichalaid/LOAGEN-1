...
import { isDateAvailable, getNextAvailableSlot,createEvent } from "../../services/scripts/calendar2.js";
...
const DEFAULT_PROFESIONAL = "BARBARA ROJAS (MICROPIGMENTACION)";
const DEFAULT_DURATION = 1.5; // 1.5 hora
const DEFAULT_SERVICIO = "Delineado de Parpados"
...
const facialflow = addKeyword(EVENTS.ACTION)
    ...
    .addAnswer("Confirmas la fecha propuesta? Responde únicamente con un 'si' o 'no'", { capture: true }, async (ctx, ctxFn) => {
        ...
        return ctxFn.gotoFlow(cita6);
    });

const cita6 = addKeyword(EVENTS.ACTION)
