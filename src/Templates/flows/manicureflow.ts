...
const DEFAULT_DURATION = 0.5; // 0.5 hora
const DEFAULT_SERVICIO = "LIFTING Y LAMINADO"
...
const manicureflow = addKeyword(EVENTS.ACTION)
    ...
    .addAnswer("Confirmas la fecha propuesta? Responde únicamente con un 'si' o 'no'", { capture: true }, async (ctx, ctxFn) => {
        ...
        return ctxFn.gotoFlow(cita);
    });

const cita = addKeyword(EVENTS.ACTION)
